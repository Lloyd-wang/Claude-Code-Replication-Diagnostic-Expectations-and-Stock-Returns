...
desc = pd.read_parquet("BGLS2019_rep/replication/output/descriptive.parquet",
                       columns=["PERMNO", "yr", "LTG", "GROW_F0"])

# Build ib_F0 and ib_F5 from compustat_eps_panel (COMPUSTAT ib per share)
ib = pd.read_parquet("BGLS2019_rep/replication/output/compustat_eps_panel.parquet")
...
fig.tight_layout()
fig.savefig("BGLS2019_rep/replication/output/Figure7.png", dpi=150)
plt.close(fig)
...
fig.tight_layout()
fig.savefig("BGLS2019_rep/replication/output/Figure8.png", dpi=150)
plt.close(fig)
