...
All scripts should be run from the repo root (`BGLS2019_rep/`), not from this directory.

## Execution Order

1. **Data download**: Run `python replication/code/00_download_data.py` to download CRSP/COMPUSTAT from WRDS into `data/` and build the COMPUSTAT EPS panel. IBES data and Fama-French factors must be obtained separately (see `data/README.md`).
2. **Independent scripts**: All numbered scripts (`11_*` through `16_*`) can be run independently.
